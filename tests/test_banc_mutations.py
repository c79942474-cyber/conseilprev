...
LE DÉFAUT QUI A FAIT ÉCRIRE CES RÈGLES, MESURÉ DANS CE DÉPÔT :
  · le script qui rejoue `tests/mutations_*.json` vivait hors du dépôt —
    personne d'autre ne pouvait vérifier qu'une règle tombe sur SON défaut ;
  · une ancre devenue ambiguë (présente deux fois dans sentinel.html) avait
    rendu une mutation inexécutable, sans qu'aucune règle ne tombe ;
  · un banc interrompu a laissé un fichier du dépôt MUTÉ sur le disque ;
...
def test_un_banc_INTERROMPU_remet_le_fichier_en_etat(tmp_path):
    """MESURÉ : un arrêt pendant une mutation a laissé un fichier du dépôt
    muté. La restauration se fait dans `finally`, et sur SIGINT/SIGTERM."""
    racine = _depot(tmp_path)
...
def test_une_modification_PENDANT_la_batterie_n_est_pas_ecrasee(tmp_path):
    """LE RISQUE, VÉCU : on corrige un fichier pendant que la batterie tourne.
...
def test_une_erreur_de_COLLECTE_est_une_chute_pas_une_survie():
    """UN MODULE QUI NE SE CHARGE PLUS A ÉTÉ ATTRAPÉ. L'ancien banc lisait
    l'erreur de collecte comme une survie, et obligeait à viser la garde."""
