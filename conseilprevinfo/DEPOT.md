...
**2. Depuis une nouvelle session** dont les sources incluent ce dépôt.

**3. En autorisant le dépôt dans la session en cours**, si l'interface vous
propose d'approuver l'ajout d'une source.

`HISTORIQUE.bundle` porte **l'historique git complet — neuf commits**,
...
`PORT`. Dépendances : `flask`, `pyyaml` (ATLAS et Electricity Maps publient en
YAML), et `pypdf` seulement si la confrontation doit lire des PDF — sans lui
elle le dit et accepte les autres formats.

Aucune dépendance vers `conseilprev` ni vers `conseilprevcyber` : ce site ne
doit pas partager leur service.

## Les neuf commits contenus dans le bundle
