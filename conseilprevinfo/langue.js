...
(function () {
  ...
  var D = {
    ...
    "bl.revue":        ["La revue", "The review"],
    "bl.revue.dit":    ["La semaine, et le mois hors de France",
                        "The week, and the month outside France"],
    ...
    /* ── L'ENTREPRISE NOMMÉE, ET LES DEUX PROVENANCES DU PAYS ────────
       « Entreprise NOMMÉE » et non « entreprise concernée » : ce site
       ne sait pas qui est concerné par une faille, il sait qui la
       source a nommé. Les deux intitulés de groupe du menu Pays disent
       laquelle des deux questions on pose — le territoire du fait, ou
       la nationalité du fournisseur. */
    "f.orga":          ["Entreprise nommée", "Company named"],
    "f.orga.vide":     ["Aucune entreprise nommée sur ces fiches",
                        "No company named on these entries"],
    "f.pays.fait":     ["Le fait s'y situe", "Where the fact takes place"],
    "f.pays.siege":    ["Siège de l'entreprise nommée",
                        "Head office of the company named"],
    "og.titre":        ["Entreprises nommées par la source — ",
                        "Companies named by the source — "],
    "og.dispute":      ["siège disputé", "head office disputed"],

    /* ── LA REVUE ───────────────────────────────────────────────────
       DEUX DÉCOUPES DU MÊME CORPUS, et une page qui dit d'abord SUR
       QUOI elle découpe. Une revue de presse qui ne le dit pas laisse
       croire qu'elle couvre l'actualité de la semaine, alors qu'elle
       couvre les faits DATÉS de cette semaine et entrés au corpus. */
    "rv.titre":        ["La revue", "The review"],
    "rv.devise":       ["Deux découpes du même corpus :",
                        "Two cuts of the same corpus:"],
    "rv.devise.b":     ["la semaine, et le mois vu hors de France",
                        "the week, and the month seen from outside France"],
    "rv.devise.fin":   [". Rien n'est réécrit pour l'occasion — chaque entrée renvoie à sa fiche, avec sa source et son statut.",
                        ". Nothing is rewritten for the occasion — every item links back to its entry, with its source and its status."],
    "rv.compte.b":     ["Ce que cette revue compte.", "What this review counts."],
    "rv.compte.t":     ["Les fiches dont LE FAIT est daté de la période — jamais celles que nous avons collectées pendant la période. Les deux dates ne coïncident pas : CISA inscrit en mars une faille exploitée depuis des mois, MITRE publie en août l'étude d'un incident de janvier. Une période sans entrée ne dit donc pas qu'il ne s'est rien passé : elle dit qu'aucun fait daté de ces jours-là n'est entré au corpus.",
                        "The entries whose FACT is dated within the period — never the ones we collected during the period. The two dates do not coincide: CISA lists in March a flaw exploited for months, MITRE publishes in August the study of a January incident. A period with no items therefore does not say that nothing happened: it says that no fact dated within those days has entered the corpus."],
    "rv.hebdo":        ["Revue hebdomadaire", "Weekly review"],
    "rv.mensuel":      ["Revue mensuelle internationale",
                        "Monthly international review"],
    "rv.prec":         ["← Période précédente", "← Previous period"],
    "rv.suiv":         ["Période suivante →", "Next period →"],
    "rv.dernier":      ["La plus récente documentée", "The most recent documented"],
    "rv.chargement":   ["Découpage du corpus…", "Cutting the corpus…"],
    "rv.panne":        ["La revue n'a pas pu être composée. ",
                        "The review could not be assembled. "],
    "rv.panne2":       ["Le corpus n'a pas répondu dans le temps imparti. Rien n'est affiché plutôt qu'une période incomplète prise pour une période creuse.",
                        "The corpus did not answer in time. Nothing is shown, rather than an incomplete period mistaken for an empty one."],
    "rv.fiches":       ["fiche(s)", "entries"],
    "rv.precedente":   ["période précédente :", "previous period:"],
    "rv.vide":         ["Aucun fait daté de cette période n'est entré au corpus. ",
                        "No fact dated within this period has entered the corpus. "],
    "rv.vide2":        ["Ce n'est pas la même chose que « il ne s'est rien passé » : les sept collecteurs ne couvrent pas tout, et cette page ne peut parler que de ce qu'ils rapportent.",
                        "That is not the same as “nothing happened”: the seven collectors do not cover everything, and this page can only speak of what they bring back."],
    "rv.retard":       ["Cette période n'est pas la semaine en cours.",
                        "This period is not the current week."],
    "rv.retard.t":     ["Elle s'est achevée il y a %j jours — c'est la plus récente que le corpus documente.",
                        "It ended %j days ago — it is the most recent one the corpus documents."],
    "rv.retard.ancienne": ["Elle s'est achevée il y a %j jours. Le corpus documente une période plus récente — « la plus récente documentée » y ramène.",
                           "It ended %j days ago. The corpus documents a more recent period — “the most recent documented” takes you back to it."],
    "rv.retard.dernier": ["Le fait le plus récent qu'il porte est daté du",
                          "The most recent fact it carries is dated"],
    /* UN CORPUS VIDE N'EST PAS UNE PÉRIODE VIDE, et les confondre est le
       pire des deux : le lecteur tirerait un jugement sur le silence des
       sources là où rien n'a encore été lu. */
    "rv.nocorpus":     ["Le corpus n'est pas chargé.",
                        "The corpus is not loaded."],
    "rv.nocorpus.t":   ["Aucune fiche n'est en mémoire — la collecte est en cours, ou elle a échoué. Ce que cette page affiche ne dit donc rien de la période : il n'y a rien à découper. Rechargez dans quelques secondes.",
                        "No entry is in memory — collection is under way, or it failed. What this page shows therefore says nothing about the period: there is nothing to cut. Reload in a few seconds."],
    "rv.regle":        ["La règle de sélection.", "The selection rule."],
    "rv.absences":     ["Ce que cette période ne dit pas",
                        "What this period does not say"],
    "rv.muets":        ["Sujets sans aucune entrée :", "Topics with no item at all:"],
    "rv.conv":         ["Dates posées, écartées :", "Set dates, left out:"],
    "rv.conv.t":       ["%n fiche(s) de cette période portent une date que ce site a posée faute de mieux — un mix annuel devient le 1er janvier. Elles tomberaient toutes dans la même semaine et gonfleraient une revue sans qu'aucun fait ne s'y soit produit.",
                        "%n entry(ies) in this period carry a date this site set for want of better — an annual mix becomes 1 January. They would all fall in the same week and inflate a review without a single fact having occurred in it."],
    "rv.hors":         ["Sans territoire, écartées :", "With no territory, left out:"],
    "rv.hors.t":       ["%n fiche(s) ne rattachent le fait à aucun pays et ne nomment aucune entreprise. Elles ne sont ni internationales ni françaises : les compter dans l'un ou l'autre serait deux fois faux.",
                        "%n entry(ies) tie the fact to no country and name no company. They are neither international nor French: counting them as either would be wrong twice over."],
    "rv.fr":           ["France seulement, écartées :", "France only, left out:"],
    "rv.fr.t":         ["%n fiche(s) ne rattachent le fait qu'à la France. Elles figurent dans la revue hebdomadaire.",
                        "%n entry(ies) tie the fact to France alone. They appear in the weekly review."],
    "rv.signees":      ["Reportages et entretiens — ce qui ne se dérive pas",
                        "Reports and interviews — what cannot be derived"],
    "rv.emporter":     ["Emporter cette revue", "Take this review with you"],
    "rv.emporter.dit": ["Le document porte ce que porte cette page, réserves comprises : ce que la revue compte, ce qu'elle écarte, et les deux rubriques vides. Rien n'y est réécrit ni résumé, et il porte l'adresse de la période qu'il rend — un fichier reçu en comité doit pouvoir rouvrir la même.",
                        "The document carries what this page carries, reservations included: what the review counts, what it leaves out, and the two empty sections. Nothing in it is rewritten or summarised, and it carries the address of the period it renders — a file received in a meeting must be able to reopen the same one."],
    "rv.pi.b":         ["Pourquoi cette revue ne commente pas.",
                        "Why this review does not comment."],
    "rv.pi.t":         ["Une revue de presse qui commente cesse d'en être une. Le classement par portée est celui du moteur, déjà publié sur chaque fiche ; les comptes sont des comptes. Aucune phrase de cette page n'apprécie ce qu'elle range — et les deux rubriques qui demanderaient une plume sont vides, signées par personne, plutôt que remplies par une machine.",
                        "A press review that comments stops being one. The ordering by reach is the engine's, already published on every entry; the counts are counts. No sentence on this page appraises what it files — and the two sections that would take a pen are empty, signed by nobody, rather than filled by a machine."],
